"""M-02 Part B (Study M2) runner — post-sweep direction & volatility.

Thin driver only: loads each burned dataset via load_candles, builds the
killzone mask exactly as the frozen Part B design specifies, and calls the
tested library (run_sweep_study / render_sweep_tables). No study logic is
re-implemented here and no parameters are changed from the library defaults
(swing_k=3, eq_tol_atr=0.25, atr_period=14, horizons=(96,384), window=96,
k=20, vol_tol=0.25, seed=42).
...
from ict_backtest.analytics.sweep_study import render_sweep_tables, run_sweep_study
...
def main() -> None:
    ...
    for label, path in DATASETS.items():
        ...
        result = run_sweep_study(candles, kz_mask=kz_mask)  # library defaults, seed=42
        table = render_sweep_tables(result, label)
        print(table)
        print()
        out[label] = {"n_bars": len(candles), "kz_bars": int(kz_mask.sum()),
                      "result": result}

    with open("reports/dev/_m2_partb_results.json", "w") as fh:
        json.dump({"killzones": list(KZ_NAMES), "killzone_tz": "America/New_York",
                   "params": {"swing_k": 3, "eq_tol_atr": 0.25, "atr_period": 14,
                              "horizons": [96, 384], "window": 96, "k": 20,
                              "vol_tol": 0.25, "seed": 42},
                   "datasets": out}, fh, indent=2)
    print("wrote reports/dev/_m2_partb_results.json")
